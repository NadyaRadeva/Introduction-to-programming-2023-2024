...
bool isMatrixDiagonal(int matrix[][MAX_SIZE], unsigned int m, unsigned int n);
...
bool isMatrixDiagonal(int matrix[][MAX_SIZE], unsigned int n, unsigned int m) {
	for (size_t i = 0; i < m; ++i) {
		for (size_t j = 0; j < n; ++j) {
			if (i == j && matrix[i][j] == 0) {
				return true;
...
int main() {
	...
	std::cout << "The matrix is diagonal? - " << std::boolalpha << isMatrixDiagonal(matrix, m, n) << std::endl;
